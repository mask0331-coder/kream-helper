...
## 현재 기능
- 설정에서 지정한 URL 패턴 + 링크 텍스트 조건을 모두 만족하는 링크를 클릭하면, 페이지 이동 대신 **팝업 창**으로 엽니다. (kream.co.kr 전용)
  - 기본값: URL이 `/products/`를 포함하고, 링크 텍스트가 `상품상세`로 시작할 때
    (URL 패턴만 쓰면 검색 결과 등 같은 URL 패턴의 링크가 페이지에 너무 많아 오작동했던 적이 있어서,
    링크 텍스트 조건을 함께 걸어 특정 링크만 정확히 골라냅니다 — 옵션에서 조정 가능)
...
- 링크 텍스트 조건 (비워두면 URL 패턴만으로 판단 — **비추천**: 페이지 전체가 오작동할 수 있음)
- 팝업 창 크기
